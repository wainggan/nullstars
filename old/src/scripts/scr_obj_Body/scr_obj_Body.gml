...
function obj_Body_fn_move_blunt(_axis, _vel, _oncollide = undefined, _pusher = undefined) {
	...
	if _axis {
		_len := instance_place_list(self.x + _vel, self.y, obj_Actor, _list, false);
	}
	else {
		_len := instance_place_list(self.x, self.y + _vel, obj_Actor, _list, false);
	}
	...
	if self.strong {
		...
		if _axis {
			_len = instance_place_list(self.x + _vel_wall, self.y, obj_Actor, _list, false);
		}
		else {
			_len = instance_place_list(self.x, self.y + _vel_wall, obj_Actor, _list, false);
		}
